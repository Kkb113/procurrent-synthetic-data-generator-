...
def test_v2_received_quantity_exceeding_shipped_fails() -> None:
    data = _valid_v2_data()
    data["GoodsReceiptLine"].loc[0, "ReceivedQuantity"] = 5.0
...
def test_v2_short_quantity_mismatch_fails() -> None:
    data = _valid_v2_data()
    data["GoodsReceiptLine"].loc[0, "ShortQuantity"] = 0.0
...
def test_v2_inspection_quantity_exceeding_received_fails() -> None:
    data = _valid_v2_data()
    data["InspectionResult"].loc[0, "InspectedQuantity"] = 4.0
    data["InspectionResult"].loc[0, "AcceptedQuantity"] = 3.5
    data["InspectionResult"].loc[0, "RejectedQuantity"] = 0.5
...
def test_v2_inventory_transaction_location_lineage_mismatch_fails() -> None:
    ...
    assert _has_issue(report, "V2_INVENTORY_LOCATION_LINEAGE")
...
def test_v2_inventory_available_value_above_on_hand_value_fails() -> None:
    data = _valid_v2_data()
    data["Inventory"].loc[0, "AvailableValue"] = 31.0
...
def test_v2_inventory_reserved_above_on_hand_fails() -> None:
    data = _valid_v2_data()
    data["Inventory"].loc[0, "ReservedQuantity"] = 4.0
    data["Inventory"].loc[0, "AvailableQuantity"] = -1.0
...
def test_v2_inventory_last_transaction_date_mismatch_fails() -> None:
    data = _valid_v2_data()
    data["Inventory"].loc[0, "LastTransactionDate"] = "2025-01-15"
...
def test_v2_inventory_last_updated_before_transaction_fails() -> None:
    data = _valid_v2_data()
    data["Inventory"].loc[0, "LastUpdatedDate"] = "2025-01-15"
...
def test_v2_received_status_with_short_quantity_fails() -> None:
    data = _valid_v2_data()
    data["GoodsReceiptHeader"].loc[0, "ReceiptStatus"] = "Received"
...
def _valid_v2_data() -> dict[str, pd.DataFrame]:
    return {
        ...
        "ComponentMaster": pd.DataFrame({"ComponentID": [10]}),
        "Plant": pd.DataFrame({"PlantID": [100]}),
        "Warehouse": pd.DataFrame({"WarehouseID": [1000, 1001], "PlantID": [100, 200]}),
        ...
        "PurchaseOrderHdr": pd.DataFrame({"PurchaseOrderID": [51], "SupplierID": [1], "PlantID": [100], "QuotationID": [31], "OrderDate": ["2025-01-04"], "ExpectedDeliveryDate": ["2025-01-15"], "POStatus": ["PartiallyReceived"], "TotalAmount": [50.0]}),
        "PurchaseOrderLine": pd.DataFrame({"PurchaseOrderLineID": [61], "PurchaseOrderID": [51], "QuotationLineID": [41], "ComponentID": [10], "OrderedQuantity": [5.0], "UnitPrice": [10.0], "LineAmount": [50.0], "LineStatus": ["PartiallyReceived"]}),
        "POSchedule": pd.DataFrame({"POScheduleID": [71], "PurchaseOrderLineID": [61], "ScheduledDeliveryDate": ["2025-01-12"], "ScheduledQuantity": [5.0], "ScheduleStatus": ["PartiallyShipped"]}),
        "ShipmentHdr": pd.DataFrame({"ShipmentID": [81], "PurchaseOrderID": [51], "SupplierID": [1], "ShipmentDate": ["2025-01-13"], "ShipmentStatus": ["PartiallyDelivered"]}),
        "ShipmentLine": pd.DataFrame({"ShipmentLineID": [91], "ShipmentID": [81], "POScheduleID": [71], "PurchaseOrderLineID": [61], "ComponentID": [10], "ShippedQuantity": [4.0]}),
        "GoodsReceiptHeader": pd.DataFrame({"GoodsReceiptID": [101], "ShipmentID": [81], "PlantID": [100], "WarehouseID": [1000], "ReceiptDate": ["2025-01-14"], "ReceiptStatus": ["ShortReceived"]}),
        "GoodsReceiptLine": pd.DataFrame({"GoodsReceiptLineID": [111], "GoodsReceiptID": [101], "ShipmentLineID": [91], "PurchaseOrderLineID": [61], "ComponentID": [10], "ShippedQuantity": [4.0], "ReceivedQuantity": [3.5], "DamagedQuantity": [0.0], "ShortQuantity": [0.5]}),
        "IncomingInspection": pd.DataFrame({"InspectionID": [121], "GoodsReceiptLineID": [111], "InspectionDate": ["2025-01-15"], "InspectionStatus": ["PartiallyRejected"]}),
        "InspectionResult": pd.DataFrame({"InspectionResultID": [131], "InspectionID": [121], "InspectedQuantity": [3.5], "AcceptedQuantity": [3.0], "RejectedQuantity": [0.5], "RejectionRatePct": [14.29], "RejectionReason": ["Surface Defect"], "ResultStatus": ["PartiallyRejected"]}),
        "InventoryTransaction": pd.DataFrame({"InventoryTransactionID": [141], "InspectionResultID": [131], "GoodsReceiptLineID": [111], "PurchaseOrderLineID": [61], "SupplierID": [1], "ComponentID": [10], "PlantID": [100], "WarehouseID": [1000], "TransactionDate": ["2025-01-16"], "TransactionType": ["StockIn"], "TransactionQuantity": [3.0], "UnitPrice": [10.0], "InventoryValue": [30.0], "ReferenceDocument": ["PO-000051-GRN-000101"], "InventoryStatus": ["Posted"]}),
        "Inventory": pd.DataFrame({"InventoryID": [171], "ComponentID": [10], "PlantID": [100], "WarehouseID": [1000], "OnHandQuantity": [3.0], "ReservedQuantity": [0.5], "AvailableQuantity": [2.5], "OnHandValue": [30.0], "AvailableValue": [25.0], "LastTransactionDate": ["2025-01-16"], "LastUpdatedDate": ["2025-01-16"], "InventoryStatus": ["Available"]}),
        "SupplierInvoice": pd.DataFrame({"SupplierInvoiceID": [151], "PurchaseOrderID": [51], "SupplierID": [1], "GoodsReceiptID": [101], "InvoiceDate": ["2025-01-17"], "DueDate": ["2025-02-16"], "InvoiceStatus": ["PartiallyPaid"], "InvoiceAmount": [35.0], "TaxAmount": [1.0], "FreightAmount": [2.0], "TotalInvoiceAmount": [38.0]}),
